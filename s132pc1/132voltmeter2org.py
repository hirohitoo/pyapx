...
while True:
    with nidaqmx.Task() as task0:
        try:
              task0.ai_channels.add_ai_voltage_chan("Dev1/ai16",terminal_config=nidaqmx.constants.TerminalConfiguration.RSE) 
              data0=task0.read(number_of_samples_per_channel=5000)   
        except:
              data0=99.99
...
#    BL132_Is=statistics.mean(data3)
    print("{:8.3f}".format(BL132_Io),end='\r')
